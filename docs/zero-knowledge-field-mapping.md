...
| Import | Clears stores one by one, so failure can leave a partial restore. | Parse with limits, encrypt/authenticate and round-trip verify before writes, then atomically promote ciphertext and clear legacy plaintext in one transaction. |
...
7. Mark `verified`; atomically commit the verified ciphertext and clear the legacy plaintext stores. Any failure aborts the transaction and retains the old plaintext.
